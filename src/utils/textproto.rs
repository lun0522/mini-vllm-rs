...
/// Formats a generated Prost message using protobuf text format.
pub(crate) fn format_textproto<M>(message: &M, message_name: &str) -> Result<String, String>
where
    M: Message,
{
    let descriptor_pool = descriptor_pool()?;
    let descriptor = descriptor_pool
        .get_message_by_name(message_name)
        .ok_or_else(|| format!("protobuf descriptor '{message_name}' is missing"))?;
    let dynamic_message = DynamicMessage::decode(descriptor, message.encode_to_vec().as_slice())
        .map_err(|error| format!("failed to decode protobuf message: {error}"))?;
